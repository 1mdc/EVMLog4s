...
class EthLogLineTest extends munit.FunSuite {
  ...
  test("should parse address") {
    ...
    assertEquals(Test(SAddress("abc")), summon[EthLogLine[Test]].parseObj(List("abc")))
  }
  test("should parse bytes") {
    case class Test(a: Array[Byte]) derives EthLogLine
    val actual = summon[EthLogLine[Test]].parseObj(List(org.web3j.utils.Numeric.toHexString("abc".getBytes)));
  ...
  test("should parse big int") {
    ...
    assertEquals(Test(BigInt("99999999999999999999")), summon[EthLogLine[Test]].parseObj(List("3939393939393939393939393939393939393939")))
  }
